...
# ===== Parallel Branch 1: News Research =====
news_fetcher = Agent(
    name = "news_fetcher",
    ...
    tools = [google_search],
    output_key = "raw_news" # Saves to state ['raw_news']
)

# ===== Parallel Branch 2: News Summarizer =====
news_summarizer = Agent(
    name = "news_summarizer",
    model = "gemini-2.0-flash",
    description = "Summarizes news articles for key insights",
    instruction=(
...
news_pipeline = SequentialAgent(
    ...
    description = "Fetches and summarizes news articles for key insights",
)
...
social_monitor = Agent(
    name = "social_monitor",
    model = "gemini-2.0-flash",
    description = "Monitor social media trends using Google Search",
    instruction = (
...
# ===== Parallel Branch 3: Sentiment Analysis =====
...
social_pipeline = SequentialAgent(
    ...
    description = "Monitors & Analyzes social media trends and sentiment",
)
...
expert_finder = Agent(
    name = "expert_finder",
    model = "gemini-2.0-flash",
    description = "Finds experts on the topic using Google Search",
    instruction = (
...
expert_pipeline = SequentialAgent(
    ...
    description = "Finds & extracts expert opinions for content",
)
...
# ============== Article Formatter ==============
article_formater = Agent(
    name = "article_formater",
    model = "gemini-2.0-flash",
    description = "Formats article for publication",
    instruction = (
...
content_publishing_system = SequentialAgent(
    ...
    sub_agents = [
        ...
        article_formater   # Phase 4: Format
    ],
    description = "Complete content publishing system that runs research, writes, edits, and formats articles"
)
